      ...
      <div className="no-print" style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
        ...
        <div className="header" style={{ flexWrap: 'wrap', gap: 6, minHeight: 'auto', padding: '10px 14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
            <span style={{ fontWeight: 900, fontSize: 16, letterSpacing: -0.5, color: 'var(--accent)', cursor: 'pointer', flexShrink: 0 }} onClick={() => navigate('/app')}>OBRAS</span>
            <button className="btn btn-secondary btn-sm" onClick={() => navigate('/app')} style={{ flexShrink: 0 }}>
              <ArrowLeft size={14} />
            </button>
          ...
          <div className="header-actions-desktop" style={{ gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {cerrado ? (
              <>
                {/* Gestión group */}
                <div style={{ display: 'flex', background: 'var(--surface2)', border: '1px solid var(--border2)', borderRadius: 6, overflow: 'hidden' }}>
                  <button className="btn btn-secondary btn-sm" style={{ borderRadius: 0, borderRight: '1px solid var(--border2)' }} onClick={() => navigate(`/app/cotizador/presupuesto/${id}/certificado`)}>
                    <FileText size={13} /> Cert.
                  </button>
                  <button className="btn btn-secondary btn-sm" style={{ borderRadius: 0, borderRight: '1px solid var(--border2)' }} onClick={() => navigate(`/app/cotizador/gantt/${id}`)}>
                    <BarChart2 size={13} /> Gantt
                  </button>
                  <button className="btn btn-secondary btn-sm" style={{ borderRadius: 0, borderRight: '1px solid var(--border2)' }} onClick={() => navigate(`/app/cotizador/presupuesto/${id}/curva`)}>
                    📈 Curva
                  </button>
                  <button className="btn btn-secondary btn-sm" style={{ borderRadius: 0 }} onClick={() => navigate(`/app/cotizador/presupuesto/${id}/materiales`)}>
                    📦 Mat.
                  </button>
                </div>
                {/* Adicional + Reabrir */}
          ...
          <div className="header-actions-mobile" style={{ gap: 6 }}>
            <MobileMenu actions={[
              ...
              ...(cerrado ? [
                { label: 'Certificados', icon: '📄', onClick: () => navigate(`/app/cotizador/presupuesto/${id}/certificado`) },
                { label: 'Gantt', icon: '📊', onClick: () => navigate(`/app/cotizador/gantt/${id}`) },
                { label: 'Curva de inversión', icon: '📈', onClick: () => navigate(`/app/cotizador/presupuesto/${id}/curva`) },
                { label: 'Listado de materiales', icon: '📦', onClick: () => navigate(`/app/cotizador/presupuesto/${id}/materiales`) },
              ...
              <div style={{ flex: 1, overflow: 'auto', paddingBottom: 80 }}>
                {data?.es_adicional && data?.presupuesto_base_id && (
                  <div style={{ margin: 10, padding: '7px 12px', background: 'rgba(110,231,183,0.06)', border: '1px solid rgba(110,231,183,0.2)', borderRadius: 6, fontSize: 11, color: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <button onClick={() => navigate('/app/cotizador/presupuesto/' + data.presupuesto_base_id)}
